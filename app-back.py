...
def loading():
    if request.method == 'POST':
        ...
        return render_template('loading.html', search=[population, others])
    else:
        return render_template('home.html')

@app.route('/finishSearch', methods=['GET', 'POST'])
def search():
    if request.method == 'POST':
        population = request.form.get('population')
        others = request.form.get('others')
        if STANDARTIZE:
            ...
            similarPopulation = difflib.get_close_matches(population, MeSH, 2)
            similarOthers = difflib.get_close_matches(others, MeSH, 2)
            similar = similarPopulation + similarOthers
            print("Used to be " + population + ", " + others)
            if len(similarPopulation) > 0:
                population = similarPopulation[0]
            if len(similarOthers) > 0:
                others = similarOthers[0]
            print("It is now " + population + ", " + others + "; the similars are " + str(similar))
        else:
            similar = []
